...
This directory contains a standalone, third-party-ready plugin package (`tooling/goodies/speech/plugins/loopback/`) designed to capture system master audio via Windows WASAPI loopback, eliminate speaker feedback using Acoustic Echo Cancellation (AEC), and stream clean PCM audio blocks into a Gemini Live multimodal session.

## Architecture & Components

1. [`loopback.py`](tooling/goodies/speech/plugins/loopback/loopback.py):
...
2. [`filter.py`](tooling/goodies/speech/plugins/loopback/filter.py):
...
3. [`plugin.py`](tooling/goodies/speech/plugins/loopback/plugin.py):
