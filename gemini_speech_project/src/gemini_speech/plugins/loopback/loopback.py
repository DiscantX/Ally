...
Provides device discovery, thread-safe streaming, and diagnostic WAV recording.
...
class SystemLoopbackCapture:
    """Captures system audio output (WASAPI loopback) using pyaudiowpatch with robust thread-safe queuing."""
    ...
    def start(self) -> None:
        """Initialize PyAudio, locate default WASAPI loopback device, and open input stream."""
    ...
    def stop(self) -> None:
        """Stop and close the audio stream and terminate PyAudio instance."""
        if self._stream:
            try:
    ...
    def poll_audio(self) -> Optional[bytes]:
        """Retrieve next available audio chunk from internal queue without blocking."""
        try:
    ...
    def level(self) -> float:
        """Return current loopback loudness (0.0 to 1.0) for visual meters."""
        with self._level_lock:
            return self._level


def record_diagnostic_wav(filepath: str = "loopback_test.wav", duration_seconds: float = 10.0) -> None:
    """Record system audio loopback to a diagnostic .wav file for verification."""
