...
from ingestion.plugins.mtga.resolver import EntityResolver, EnumResolver
...
class MTGALogParser:
    ...
    def _resolve_object(self, obj: dict) -> None:
        """Attaches resolved_card (if this is a real card) or
        resolved_source_card (if it's a non-card engine object -- ability,
        trigger, emblem, etc. -- generated by one) to `obj` in place.
    
        Allowlisted on type == GameObjectType_Card rather than blocklisting
        known non-card types, so an object type this log sample never
        exercised (there are more than just Ability -- Emblem, Effect, and
        others exist in the schema) degrades safely into "try the source
        instead" rather than silently mis-resolving again.
    
        `type` isn't necessarily re-sent on every diff fragment for an
        object we've already seen in full once, so this checks the merged
        state first and only falls back to the fragment's own `type` if the
        object is new this turn.
        """
        obj_id = obj.get("instanceId") or obj.get("id")
        existing = self.game_state["game_objects"].get(obj_id, {})
        obj_type = obj.get("type", existing.get("type"))
    
        grp_id = obj.get("grpId")
        if grp_id is None:
            return
    
        if obj_type is None or obj_type == self.CARD_OBJECT_TYPE:
            obj["resolved_card"] = self.entity_resolver.resolve_card(
                int(grp_id), name_loc_id=obj.get("name")
            )
            return
    
        # Non-card engine object -- grpId isn't in the Cards namespace at
        # all, so don't bother asking. Surface the card that generated it
        # instead, if we have one -- more useful than a bare "unresolved".
        source_grp_id = obj.get("objectSourceGrpId")
        if source_grp_id is not None:
            obj["resolved_source_card"] = self.entity_resolver.resolve_card(int(source_grp_id))
